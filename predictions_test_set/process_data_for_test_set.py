...
def process_data(split_prec: dict = {
    ...
    #need to modify this code so we can run it for the final predictions
    data_path_test = os.path.join(project_dir,'data', 'test_data_fall2024.csv')
    ...
    #for the test set
    processed_df_test = create_new_features(processed_df_test, info=True,dropped_columns=dropped_columns)
    ...
    if type(scaler) == skl_pre._data.StandardScaler or type(scaler) == skl_pre._data.MinMaxScaler:
        ...
        #we only have training data and we need to trasnform the test data set
        if len(splits) < 4:
            #need to reset the index of the training
            splits[1] = (splits[1].reset_index()).drop("index",axis = 1)
            splits.append(pd.DataFrame(scaler.transform(processed_df_test),columns=x_cols))
